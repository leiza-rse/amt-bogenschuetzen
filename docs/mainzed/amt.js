    ...
    loadGraph(function (graph) {
      ...
      if (modified) {
        ...
        for (var i in GRAPH.edited.nodes) {
          insert +=
            "<" +
            GRAPH.edited.nodes[i].id +
            "> amt:instanceOf <" +
            GRAPH.edited.nodes[i].concept +
            "> . ";
          insert +=
            "<" +
            GRAPH.edited.nodes[i].id +
            '> rdfs:label "' +
            GRAPH.edited.nodes[i].label +
            '" . ';
        }
        for (var i in GRAPH.edited.edges) {
          insert +=
            "_:" + i + " rdf:subject <" + GRAPH.edited.edges[i].from + "> . ";
          insert +=
            "_:" + i + " rdf:predicate <" + GRAPH.edited.edges[i].role + "> . ";
          insert +=
            "_:" + i + " rdf:object <" + GRAPH.edited.edges[i].to + "> . ";
          insert +=
            "_:" + i + " amt:weight " + GRAPH.edited.edges[i].width + " . ";
        }
        var query = "";
        query +=
          " DELETE { ?id amt:instanceOf ?concept . ?id rdfs:label ?label . } WHERE { ?concept rdf:type amt:Concept . ?id amt:instanceOf ?concept . ?id rdfs:label ?label . } ; ";
        query +=
          " DELETE { ?stmt rdf:subject ?from . ?stmt rdf:predicate ?role . ?stmt rdf:object ?to . ?stmt amt:weight ?width . } WHERE { ?role rdf:type amt:Role . ?stmt rdf:subject ?from . ?stmt rdf:predicate ?role . ?stmt rdf:object ?to . ?stmt amt:weight ?width . } ; ";
